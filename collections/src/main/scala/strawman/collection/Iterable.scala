...
trait IterableOps[+A, +CC[_], +C] extends Any with IterableOnce[A] {
  ...
  def toMap[T, U](implicit ev: A <:< (T, U)): immutable.Map[T, U] =
    immutable.Map.from(toIterable.asInstanceOf[Iterable[(T, U)]])

  def toSet[B >: A]: immutable.Set[B] = immutable.Set.from(toIterable)

  def toSeq[B >: A]: immutable.Seq[B] = immutable.Seq.from(toIterable)

  def toIndexedSeq[B >: A]: immutable.IndexedSeq[B] = immutable.IndexedSeq.from(toIterable)
